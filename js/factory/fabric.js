angular.module('lepota.fabric', [
	'lepota.fabric.window',
	'lepota.fabric.directive',
	'lepota.fabric.canvas',
	'lepota.fabric.dirtyStatus'
])
...
.factory('Fabric', [
	...
	return function(options) {
		...
		self.createPrintArea = function(config) {
			var canvasWidth = canvas.getWidth();
			var canvasHeight = canvas.getHeight();


			var pa_width = parseInt( config.pa_width ) / 100 * canvasWidth, 
				pa_height = parseInt( config.pa_height ) / 100 * canvasHeight,

				pa_left = parseInt( config.pa_left ) / 100 * canvasWidth,
				pa_top = parseInt( config.pa_top ) / 100 * canvasHeight,

				width = pa_width - 2,
				height = pa_height - 2,

				stroke = ( config && config.pa_stroke ) ? config.pa_stroke : '#000000';


			canvas.clipTo = function(ctx) {
				printArea = ctx;
				ctx.beginPath();
  				ctx.rect(pa_left, pa_top, width, height);
  				ctx.closePath();
		...
		self.init = function() {
			...
			JSONObject.background = JSONObject.background || 'rgba(0, 0, 0, 0)';
			self.setCanvasBackgroundColor(JSONObject.background);

			// Set the size of the canvas
			JSONObject.width = JSONObject.width || 650;
			self.canvasOriginalWidth = JSONObject.width;

			JSONObject.height = JSONObject.height || 650;
